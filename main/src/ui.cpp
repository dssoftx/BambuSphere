...
// True when `text` is one of error_lookup.cpp's raw/unresolved diagnostic
// fallbacks (format_generic_hms_detail() / format_generic_print_error_detail()
// / format_generic_hms_count_detail()) rather than an actual looked-up
// message — i.e. an HMS/error code with no entry in the embedded lookup
// table (seen in the wild on newer models like the H2D). That raw hex dump
// carries no useful information for the user, so it shouldn't win over
// showing the printer's name on the main page (see the detail_text()
// fallback below). Matched by exact prefix/length since these formats are
// generated by fixed snprintf patterns.
bool is_unresolved_diagnostic_code_text(const std::string& text) {
  if (text.rfind("HMS alerts: ", 0) == 0) {
    return true;
  }
  if (text.rfind("HMS ", 0) == 0 && text.size() == 21 && text[12] == '_') {
    return true;
  }
  if (text.rfind("Print error ", 0) == 0 && text.size() == 21 && text[16] == '_') {
    return true;
  }
  return false;
}
...
void Ui::apply_snapshot_locked(const PrinterSnapshot& snapshot, bool force_ring_refresh) {
  ...
  // Also fall back when detail_text() only found an unresolved HMS/error
  // code with no lookup-table entry (renders as a raw hex dump — seen on
  // the H2D — which is worth less to the user than the printer's name and
  // would otherwise permanently hide it).
  std::string detail = detail_text(snapshot);
  if (detail.empty() || is_unresolved_diagnostic_code_text(detail)) {
...
esp_err_t Ui::build_dashboard() {
  ...
  // dosis_32, not dosis_40: dual-nozzle printers (e.g. H2D) prefix this with
  // "L "/"R ", and the wider dosis_40 string wrapped to a second line inside
  // the fixed box below. CLIP is a belt-and-suspenders guard against any
  // string that still doesn't fit — it clips instead of wrapping/splitting.
  ...
  // Fixed box (mirrors bed_value_label_ below) so the font has a
  ...
  // dosis_32 to match nozzle_value_label_ (see its comment above).
  ...
  lv_label_set_long_mode(bed_value_label_, LV_LABEL_LONG_CLIP);
